...
func TestHashJsonValidation(t *testing.T) {
	var tests = []struct {
		...
		{"", 62, "json: cannot unmarshal hex string without G prefix into Go value of type common.Hash"},
		{"G", 66, "hex string has length 66, want 64 for common.Hash"},
		{"G", 63, "json: cannot unmarshal hex string of odd length into Go value of type common.Hash"},
		{"G", 0, "hex string has length 0, want 64 for common.Hash"},
		{"G", 64, ""},
		{"G", 64, ""},
	}
	for _, test := range tests {
...
func TestMixedcaseAccount_Address(t *testing.T) {
	...
	// Note: G{checksum_addr} is not valid according to spec above
	...
	for _, r := range []string{
		...
		`["1111111111111111111112222222222223333323"]`,     // Missing G
