...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gec is "gec". If no
...
// These resources are resolved differently for "gec" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gec 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGethResourceWarning {
				...
				log.Warn("Using deprecated resource file, please move this file to the 'gec' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
