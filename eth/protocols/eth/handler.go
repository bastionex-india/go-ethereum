...
// MakeProtocols constructs the P2P protocol definitions for `G`.
...
// NodeInfo represents a short summary of the `G` sub-protocol metadata
...
// nodeInfo retrieves some `G` protocol metadata about the running host node.
...
// Handle is invoked whenever an `G` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `G`", "err", err)
			return err
		}
	}
}
