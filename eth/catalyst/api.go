...
func Register(stack *node.Node, backend *G.Ethereum) error {
	log.Warn("Engine API enabled", "protocol", "G")
...
func NewConsensusAPI(G *G.Ethereum) *ConsensusAPI {
	if G.BlockChain().Config().TerminalTotalDifficulty == nil {
	...
	G.Downloader().SetBadBlockCallback(api.setInvalidAncestor)
	go api.heartbeat()
...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	if payloadAttributes != nil {
		...
		if api.G.BlockChain().Config().IsShanghai(payloadAttributes.Timestamp) {
...
func (api *ConsensusAPI) verifyPayloadAttributes(attr *engine.PayloadAttributes) error {
	if !api.G.BlockChain().Config().IsShanghai(attr.Timestamp) {
...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	...
	// reason.
	block := api.G.BlockChain().GetBlockByHash(update.HeadBlockHash)
	if block == nil {
		...
		// the future to resolve from the `G` network, but it's an unexpected case
		...
		if merger := api.G.Merger(); !merger.TDDReached() {
			merger.ReachTTD()
			api.G.Downloader().Cancel()
		}
		...
		if err := api.G.Downloader().BeaconSync(api.G.SyncMode(), header, finalized); err != nil {
	...
	if block.Difficulty().BitLen() > 0 || block.NumberU64() == 0 {
		var (
			td  = api.G.BlockChain().GetTd(update.HeadBlockHash, block.NumberU64())
			ptd = api.G.BlockChain().GetTd(block.ParentHash(), block.NumberU64()-1)
			ttd = api.G.BlockChain().Config().TerminalTotalDifficulty
		)
	...
	if rawdb.ReadCanonicalHash(api.G.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		// Block is not canonical, set head.
		if latestValid, err := api.G.BlockChain().SetCanonical(block); err != nil {
			return engine.ForkChoiceResponse{PayloadStatus: engine.PayloadStatusV1{Status: engine.INVALID, LatestValidHash: &latestValid}}, err
		}
	} else if api.G.BlockChain().CurrentBlock().Hash() == update.HeadBlockHash {
		...
		log.Info("Ignoring beacon update to old head", "number", block.NumberU64(), "hash", update.HeadBlockHash, "age", common.PrettyAge(time.Unix(int64(block.Time()), 0)), "have", api.G.BlockChain().CurrentBlock().Number)
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		if merger := api.G.Merger(); !merger.PoSFinalized() {
			merger.FinalizePoS()
		}
		// If the finalized block is not in our canonical tree, somethings wrong
		finalBlock := api.G.BlockChain().GetBlockByHash(update.FinalizedBlockHash)
		if finalBlock == nil {
			log.Warn("Final block not available in database", "hash", update.FinalizedBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not available in database"))
		} else if rawdb.ReadCanonicalHash(api.G.ChainDb(), finalBlock.NumberU64()) != update.FinalizedBlockHash {
			log.Warn("Final block not in canonical chain", "number", block.NumberU64(), "hash", update.HeadBlockHash)
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("final block not in canonical chain"))
		}
		// Set the finalized block
		api.G.BlockChain().SetFinalized(finalBlock.Header())
	}
	// Check if the safe block hash is in our canonical tree, if not somethings wrong
	if update.SafeBlockHash != (common.Hash{}) {
		safeBlock := api.G.BlockChain().GetBlockByHash(update.SafeBlockHash)
		if safeBlock == nil {
			log.Warn("Safe block not available in database")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not available in database"))
		}
		if rawdb.ReadCanonicalHash(api.G.ChainDb(), safeBlock.NumberU64()) != update.SafeBlockHash {
			log.Warn("Safe block not in canonical chain")
			return engine.STATUS_INVALID, engine.InvalidForkChoiceState.With(errors.New("safe block not in canonical chain"))
		}
		// Set the safe block
		api.G.BlockChain().SetSafe(safeBlock.Header())
	}
	...
	if payloadAttributes != nil {
		...
		payload, err := api.G.Miner().BuildPayload(args)
		if err != nil {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config engine.TransitionConfigurationV1) (*engine.TransitionConfigurationV1, error) {
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "gec", ttd, "beacon", config.TerminalTotalDifficulty)
		return nil, fmt.Errorf("invalid ttd: execution %v consensus %v", ttd, config.TerminalTotalDifficulty)
	}
	if config.TerminalBlockHash != (common.Hash{}) {
		if hash := api.G.BlockChain().GetCanonicalHash(uint64(config.TerminalBlockNumber)); hash == config.TerminalBlockHash {
...
func (api *ConsensusAPI) NewPayloadV2(params engine.ExecutableData) (engine.PayloadStatusV1, error) {
	if api.G.BlockChain().Config().IsShanghai(params.Timestamp) {
		if params.Withdrawals == nil {
...
func (api *ConsensusAPI) newPayload(params engine.ExecutableData) (engine.PayloadStatusV1, error) {
	...
	//      api.G.BlockChain().InsertBlockWithoutSetHead, where it is blocked on
	...
	if block := api.G.BlockChain().GetBlockByHash(params.BlockHash); block != nil {
	...
	parent := api.G.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		ptd  = api.G.BlockChain().GetTd(parent.Hash(), parent.NumberU64())
		ttd  = api.G.BlockChain().Config().TerminalTotalDifficulty
		gptd = api.G.BlockChain().GetTd(parent.ParentHash(), parent.NumberU64()-1)
	)
	if ptd.Cmp(ttd) < 0 {
	...
	if api.G.SyncMode() != downloader.FullSync {
		return api.delayPayloadImport(block)
	}
	if !api.G.BlockChain().HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
		api.remoteBlocks.put(block.Hash(), block.Header())
		log.Warn("State not available, ignoring new payload")
		return engine.PayloadStatusV1{Status: engine.ACCEPTED}, nil
	}
	log.Trace("Inserting block without sethead", "hash", block.Hash(), "number", block.Number)
	if err := api.G.BlockChain().InsertBlockWithoutSetHead(block); err != nil {
...
func (api *ConsensusAPI) delayPayloadImport(block *types.Block) (engine.PayloadStatusV1, error) {
	...
	if err := api.G.Downloader().BeaconExtend(api.G.SyncMode(), block.Header()); err == nil {
...
func (api *ConsensusAPI) checkInvalidAncestor(check common.Hash, head common.Hash) *engine.PayloadStatusV1 {
	...
	if header := api.G.BlockChain().GetHeader(invalid.ParentHash, invalid.Number.Uint64()-1); header != nil && header.Difficulty.Sign() != 0 {
...
func (api *ConsensusAPI) invalid(err error, latestValid *types.Header) engine.PayloadStatusV1 {
	currentHash := api.G.BlockChain().CurrentBlock().Hash()
	if latestValid != nil {
...
func (api *ConsensusAPI) heartbeat() {
	...
	for {
		...
		if api.G.BlockChain().Config().TerminalTotalDifficultyPassed || api.G.Merger().TDDReached() {
...
func (api *ConsensusAPI) GetPayloadBodiesByHashV1(hashes []common.Hash) []*engine.ExecutionPayloadBodyV1 {
	...
	for i, hash := range hashes {
		block := api.G.BlockChain().GetBlockByHash(hash)
...
func (api *ConsensusAPI) GetPayloadBodiesByRangeV1(start, count hexutil.Uint64) ([]*engine.ExecutionPayloadBodyV1, error) {
	...
	current := api.G.BlockChain().CurrentBlock().Number.Uint64()
	...
	for i := uint64(start); i <= last; i++ {
		block := api.G.BlockChain().GetBlockByNumber(i)
