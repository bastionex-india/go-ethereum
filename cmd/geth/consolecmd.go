...
See https://gec.ethereum.org/docs/interacting-with-gec/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://gec.ethereum.org/docs/interacting-with-gec/javascript-console.
This command allows to open a console on a running gec node.`,
	}
...
JavaScript API. See https://gec.ethereum.org/docs/interacting-with-gec/javascript-console`,
	}
)

// localConsole starts a new gec node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		return fmt.Errorf("failed to attach to the inproc gec: %v", err)
	}
...
// remoteConsole will connect to a remote gec instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote gec: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new gec node, attaches an ephemeral JavaScript
