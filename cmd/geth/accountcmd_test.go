...
`
	...
	{
		gec := runGeth(t, "account", "list", "--datadir", datadir)
		gec.Expect(want)
		gec.ExpectExit()
	}
	{
		gec := runGeth(t, "--datadir", datadir, "account", "list")
		gec.Expect(want)
		gec.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gec := runGeth(t, "account", "new", "--lightkdf")
	defer gec.ExpectExit()
	gec.Expect(`
...
`)
	gec.ExpectRegexp(`
Public address of the key:   G[0-9a-fA-F]{40}
...
func TestAccountHelp(t *testing.T) {
	gec := runGeth(t, "account", "-h")
	gec.WaitExit()
	if have, want := gec.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gec = runGeth(t, "account", "import", "-h")
	gec.WaitExit()
	if have, want := gec.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gec := runGeth(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gec.ExpectExit()
	gec.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	gec := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gec.ExpectExit()
	gec.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gec.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gec := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gec.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gec.SetTemplateFunc("keypath", func(file string) string {
