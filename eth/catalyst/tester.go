...
func RegisterFullSyncTester(stack *node.Node, backend *G.Ethereum, block *types.Block) (*FullSyncTester, error) {
	cl := &FullSyncTester{
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if tester.api.G.Downloader().Synchronising() {
					continue
				}
				// Short circuit in case the target block is already stored
				// locally. TODO(somehow terminate the node stack if target
				// is reached).
				if tester.api.G.BlockChain().HasBlock(tester.block.Hash(), tester.block.NumberU64()) {
				...
				// trusted chain head.
				err := tester.api.G.Downloader().BeaconSync(downloader.FullSync, tester.block.Header(), nil)
				if err != nil {
