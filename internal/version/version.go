...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gec".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gec %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gec %s", params.VersionWithMeta)
	}
