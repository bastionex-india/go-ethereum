...
// Package gethclient provides an RPC client for gec-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements gec-specific functionality.
//
...
// GCStats retrieves the current garbage collection stats from a gec node.
...
// MemStats retrieves the current memory stats from a gec node.
...
// GetNodeInfo retrieves the node info of a gec node.
