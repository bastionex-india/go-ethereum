...
var defaultAPIs = map[string]string{"G": "1.0", "net": "1.0", "debug": "1.0"}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines G.getBalance(G.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	if res, err := c.jsre.Run(`
		...
		try {
			message += "coinbase: " + G.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + G.blockNumber + " (" + new Date(1000 * G.getBlock(G.blockNumber).timestamp) + ")\n";
		try {
