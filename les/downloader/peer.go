...
type peerConnection struct {
	...
	version uint       // G protocol version number to switch strategies
...
func (p *peerConnection) SetHeadersIdle(delivered int, deliveryTime time.Time) {
	p.rates.Update(G.BlockHeadersMsg, deliveryTime.Sub(p.headerStarted), delivered)
	atomic.StoreInt32(&p.headerIdle, 0)
}
...
func (p *peerConnection) SetBodiesIdle(delivered int, deliveryTime time.Time) {
	p.rates.Update(G.BlockBodiesMsg, deliveryTime.Sub(p.blockStarted), delivered)
	atomic.StoreInt32(&p.blockIdle, 0)
}
...
func (p *peerConnection) SetReceiptsIdle(delivered int, deliveryTime time.Time) {
	p.rates.Update(G.ReceiptsMsg, deliveryTime.Sub(p.receiptStarted), delivered)
	atomic.StoreInt32(&p.receiptIdle, 0)
}
...
func (p *peerConnection) SetNodeDataIdle(delivered int, deliveryTime time.Time) {
	p.rates.Update(G.NodeDataMsg, deliveryTime.Sub(p.stateStarted), delivered)
	atomic.StoreInt32(&p.stateIdle, 0)
}
...
func (p *peerConnection) NodeDataCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(G.NodeDataMsg, targetRTT)
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		rates: msgrate.NewTrackers(log.New("proto", "G")),
	}
}
...
func (ps *peerSet) HeaderIdlePeers() ([]*peerConnection, int) {
	...
	throughput := func(p *peerConnection) int {
		return p.rates.Capacity(G.BlockHeadersMsg, time.Second)
	}
	return ps.idlePeers(G.ETH66, G.ETH67, idle, throughput)
}
...
func (ps *peerSet) BodyIdlePeers() ([]*peerConnection, int) {
	...
	throughput := func(p *peerConnection) int {
		return p.rates.Capacity(G.BlockBodiesMsg, time.Second)
	}
	return ps.idlePeers(G.ETH66, G.ETH67, idle, throughput)
}
...
func (ps *peerSet) ReceiptIdlePeers() ([]*peerConnection, int) {
	...
	throughput := func(p *peerConnection) int {
		return p.rates.Capacity(G.ReceiptsMsg, time.Second)
	}
	return ps.idlePeers(G.ETH66, G.ETH67, idle, throughput)
}
...
func (ps *peerSet) NodeDataIdlePeers() ([]*peerConnection, int) {
	...
	throughput := func(p *peerConnection) int {
		return p.rates.Capacity(G.NodeDataMsg, time.Second)
	}
	return ps.idlePeers(G.ETH66, G.ETH67, idle, throughput)
}
